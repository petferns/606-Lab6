 ...
 **Answer**
    ...
    Count across categories is 827.
    
```{r}
texting_while_driving <- yrbss %>%
  filter( text_while_driving_30d=="30")
summary(texting_while_driving)
...
Error margin between 0.0650 to 0.0773
...
Relationship between "p" and "me" is parabolic
for p=0.50 the "me" is maximum

```{r me-plot-2, fig.show='asis'}
dd <- data.frame(p = p, me = me)
ggplot(data = dd, aes(x = p, y = me)) + 
  geom_line() +
  labs(x = "Population Proportion", y = "Margin of Error")
...
For n=300, p=0.1, distribution is almost a bell curve with center at 0.10 and shape is bell curve.
...
With constant "n" and changing "p" the spread widens.
    
1.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

**Answer** 

With change in "n" the spread narrows and the distribution tends towards more a normal distribution
